...
def get_product(request, slug):
    ...
    related_products = list(product.category.products.exclude(uid=product.uid))
...
def edit_review(request, review_uid):
    ...
    if request.method == "POST":
        ...
        review.save()
        messages.success(request, "评价已成功更新。")
...
def delete_review(request, slug, review_uid):
    if not request.user.is_authenticated:
        messages.warning(request, "请先登录再删除评价。")
...
def add_to_wishlist(request, uid):
    ...
    if created:
        ...
        messages.info(request, "该课程已在收藏列表中。")
...
def remove_from_wishlist(request, uid):
    ...
    messages.success(request, "已从收藏中移除。")
    return redirect(reverse('wishlist'))
...
def move_to_cart(request, uid):
    ...
    if not wishlist:
        messages.error(request, "收藏中未找到该课程。")
    ...
    messages.success(request, "已移入购物车！")
